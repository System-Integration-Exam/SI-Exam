...
def retrieve_users_reservation():
    try:
        return reservation_client.retrieve_users_reservation(request.json["user_id"])
    except Exception as e:
        print(e)
        return "500"
...
def collect_routes(app):
    app.add_url_rule("/reservation", view_func=create_reservation, methods=["POST"])
    app.add_url_rule(
    ...
    app.add_url_rule(
        "/reservation", view_func=retrieve_users_reservation, methods=["GET"]
    )
