...
def collect_routes(app):
    ...
    app.add_url_rule("/store/add-book=<int:store_id>&<int:book_id>", view_func=add_book_to_store, methods=["POST"])
    app.add_url_rule("/store/remove-book=<int:store_id>&<int:book_id>", view_func=remove_book_from_store, methods=["DELETE"])
    app.add_url_rule("/store/total=<int:store_id>&<int:book_id>", view_func=get_amount_of_specific_book_from_store, methods=["GET"])
    app.add_url_rule("/store", view_func=read_store_list, methods=["GET"])
    app.add_url_rule("/store/add-vinyl=<int:store_id>&<int:vinyl_id>", view_func=add_vinyl_to_store, methods=["POST"])
    app.add_url_rule("/store/remove-vinyl=<int:store_id>&<int:vinyl_id>", view_func=remove_vinyl_from_store, methods=["DELETE"])
    app.add_url_rule("/store/total=<int:store_id>&<int:vinyl_id>", view_func=get_amount_of_specific_vinyl_from_store, methods=["GET"])
